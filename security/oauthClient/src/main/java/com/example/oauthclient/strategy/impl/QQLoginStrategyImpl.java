...
import com.example.oauthclient.config.QQConfigProperties;
import com.example.oauthclient.constant.SocialLoginConst;
...
import static com.example.oauthclient.constant.SocialLoginConst.*;
import static com.example.oauthclient.enums.StatusCodeEnum.QQ_LOGIN_ERROR;
    ...
    private void checkQQToken(QQLoginVO qqLoginVO) {
        ...
        try {
            ...
            if (!qqLoginVO.getOpenId().equals(qqTokenDTO.getOpenid())) {
                throw new RuntimeException(QQ_LOGIN_ERROR.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException(QQ_LOGIN_ERROR.toString());
