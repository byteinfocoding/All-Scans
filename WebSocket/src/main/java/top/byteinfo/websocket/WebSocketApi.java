...
import top.byteinfo.websocket.model.*;
import top.byteinfo.websocket.strategy.context.UploadStrategyContext;
...
import static top.byteinfo.websocket.model.ChatTypeEnum.*;

@Data
@Service
@ServerEndpoint(value = "/websocket",configurator= WebSocketApi.ChatConfigurator.class)
public class WebSocketApi {
     ...
     * 用户session集合
     */
    private static CopyOnWriteArraySet<WebSocketApi> webSocketSet = new CopyOnWriteArraySet<>();
...
//        chatRecordDao.insert(chatRecord);
    ...
    private void broadcastMessage(WebsocketMessageDTO messageDTO) throws IOException {
        for (WebSocketApi webSocketService : webSocketSet) {
